...
export default function UpdateDataModal({
    ...
    const [myApp, setMyApp] = useState(existingMyApp || '')
    const [competitorApps, setCompetitorApps] = useState<string[]>(existingCompetitors || [])
            ...
            complete: (results) => {
                if (results.meta.fields) {
                    ...
                    if (foundAppNameCol && results.data.length > 0) {
                        ...
                        // Smart pre-fill: if existing apps aren't found in this file, guess them
                        if (!existingMyApp || !uniqueAppsArr.includes(existingMyApp)) {
                        ...
                        if (!existingCompetitors || existingCompetitors.length === 0) {
                            if (uniqueAppsArr.length > 0) {
                                setCompetitorApps(uniqueAppsArr.slice(1, 11))
                            }
                        }
                    }
                }
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await fetch(`/api/datasets/${datasetId}/update`, {
                method: 'POST',
            ...
            if (res.ok) {
                toast.success(`Updated ${json.totalUpserted} keywords successfully!`)

                // Automatically recompute scores
                toast.loading('Recomputing scores...', { id: 'recompute-toast' })
                await fetch(`/api/datasets/${datasetId}/recompute`, { method: 'POST' })
                toast.success('Scores recomputed!', { id: 'recompute-toast' })
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button variant="outline" className="gap-2 h-9 text-gray-700 bg-white border-gray-300">
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>Update Dataset: {datasetName}</DialogTitle>
                    <DialogDescription>Upload a fresh CSV to update volumes, difficulties, or ranks.</DialogDescription>
                </DialogHeader>

                <div className="space-y-6">
                    <div className="grid gap-2 relative">
                        <Label>1. Drop New CSV File</Label>
                        ...
                        <div className="space-y-4 animate-in fade-in">
                            ...
                            >
                                {isProcessing ? (
                                    ...
                                    `Update ${parsedData.length} Keywords`
                                )}
                            </Button>
                        </div>
                    )}
